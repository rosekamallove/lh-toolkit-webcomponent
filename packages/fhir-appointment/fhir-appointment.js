...
class FhirAppointment extends LitElement {
  ...
  appointmentTypeTemplate() {
    ...
    return this.showAppointmentType !== "false" ? html`
      <fhir-codeable-concept label="Appintment-Type :" .value='${this.value.appointmentType}' @input="${this.setAppointmentType}"></fhir-codeable-concept>
  ` : "";
  }
  ...
  slotTemplate() {
    ...
    return this.showSlot !== "false" ? html`
      ${this.value.slot.map((item, index) => html`
        <fhir-reference .value="${item}" label="Slot:" @input="${e => this.setSlotValue(e, index)}"></fhir-reference>
      `)}
    ` : "";
  }
  ...
  participantTemplate() {
    ...
    ${this.value.participant.map((item, index) => html`
      ${(item.type || [{coding: [{ system: "", code: "", display: ""}],text: ""}]).map((item1, index1) => html`
          <fhir-codeable-concept label="Type :" .value='${item1}' @input="${e => item.type[index1] = e.target.value}"></fhir-codeable-concept>
      `)}

      <mwc-formfield label="Status :" class="field" alignEnd>
        <mwc-textfield label="Text" .value='${item.status || ""}' @input="${e => this.value.participant[index].status = e.target.value}"> </mwc-textfield>
      </mwc-formfield>

      <fhir-reference .value="${item.actor || {reference: "", display: "", type: ""}}" label="Actor:" @input="${e => this.value.participant[index].actor = e.target.value}"></fhir-reference>
      <hr class="rule">
    `)}
  `;
  }
